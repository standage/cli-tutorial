#!/usr/bin/env Rscript
# demo.R: template for an R command-line interface
library('getopt');
...
# it is very uncommon to parse input line-by-line in R, so we'll assume the
# input is a CSV file and load it into a dataframe; if you do want/need to parse
# input line-by-line, you may want to consider whether R is the right tool for
# the job
data <- read.csv(opt$in, header=FALSE)
